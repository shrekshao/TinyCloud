...
int MasterNode::create_user(string username) {
    ...
    int index = 0;
    int min_index = 0;
    long min_load = 9999999999;
    for (auto it = mem_info_mapping.begin(); it != mem_info_mapping.end(); ++it)
    {
        if (it->second.buffer_length() < min_load) {
            min_load = it->second.buffer_length();
            min_index = index;
        }
        index++;
    }

    if (min_index == mapping_node_id || mem_info_mapping[ip_mapping[mapping_node_id]].buffer_length() <= min_load*2) {
